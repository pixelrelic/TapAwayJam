// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'

Shader "Procedural Eyes/PupilAA"
{

	SubShader
	{
		Tags {
			"RenderType"="Transparent"
			"Queue" = "Transparent"
			"PreviewType" = "Plane"
		}


		Pass
		{
			Blend SrcAlpha OneMinusSrcAlpha
            Cull Off

			CGPROGRAM
			#pragma vertex vert
			#pragma fragment frag

			
			#include "UnityCG.cginc"

			struct appdata
			{
				float4 vertex : POSITION;
                float2 uv0 : TEXCOORD0;
                float2 uv1 : TEXCOORD1;
                float2 uv2 : TEXCOORD2;
                float4 mask : TANGENT;
                float4 col : COLOR;
                //float3 cutOff : NORMAL;
			};

			struct v2f
			{
				float4 pos : SV_POSITION;
                float2 uv0 : TEXCOORD0;
                float2 uv1 : TEXCOORD1;
                float2 uv2 : TEXCOORD2;
                float4 mask : TANGENT;
                float4 col : COLOR;
                //float3 cutOff : NORMAL;
			};

			
			v2f vert (appdata v)
			{
				v2f o = (v2f)0;
                o.uv0 = v.uv0;
                o.uv1 = v.uv1;
                o.uv2 = v.uv2;
                o.col = v.col;
                o.mask = v.mask;
                //o.cutOff = v.cutOff;
                o.pos = UnityObjectToClipPos(v.vertex );
                #ifdef PIXELSNAP_ON
                    o.pos = UnityPixelSnap(o.pos);
                #endif
                return o;
			}
			
			fixed4 frag (v2f i) : SV_Target
			{
				//Circle shape (dist from center)
                float radius = length(i.uv0);
                //Clip to eye's radius
                float mask1 = length(i.mask.rg);

                //Clip based on how closed the eye is
                float mask2 = max(i.mask.b, i.mask.a);

                //float mask3 = distance(i.uv0, float2(i.cutOff.x, i.cutOff.y));

                float clipping = max(radius, mask1);

                //Antialiasing
                float aaf = fwidth(radius);

                //float aaf2 = fwidth(mask3);


                mask2 = 1-floor(mask2);
                float alpha = mask2*smoothstep(1, 1-aaf, clipping);
                //alpha *= smoothstep(i.cutOff.z-aaf2, i.cutOff.z, mask3);

                return fixed4(i.col.rgb, alpha);
			}
			ENDCG
		}
	}
}
