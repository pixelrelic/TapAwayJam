...
namespace SupersonicWisdomSDK.Editor
{
    internal static class SwSelfUpdateEventsHandler
    {
        ...
        private static void TryAlertWisdomUpdateFinished()
        {
            ...
            if (!didCompleteUpdate) return;
            
            OnSuccessfulUpdateTrackingAndModification(updateInfoJsonDictionary);
        ...
        private static void OnSuccessfulUpdateTrackingAndModification(IDictionary<string, object> updateInfoJsonDictionary)
        {
            var oldStageNumber = updateInfoJsonDictionary.SwSafelyGet(SwEditorConstants.SwKeys.OLD_SDK_STAGE_NUMBER, SwSelfUpdate.NONE_CODE.ToString()).ToString();
            var oldVersionId = updateInfoJsonDictionary.SwSafelyGet(SwEditorConstants.SwKeys.OLD_SDK_VERSION_ID, SwSelfUpdate.NONE_CODE.ToString()).ToString();
            var updatedStageNumber = updateInfoJsonDictionary.SwSafelyGet(SwEditorConstants.SwKeys.UPDATED_SDK_STAGE_NUMBER, SwSelfUpdate.NONE_CODE.ToString()).ToString();
            var updatedVersionId = updateInfoJsonDictionary.SwSafelyGet(SwEditorConstants.SwKeys.UPDATED_SDK_VERSION_ID, SwSelfUpdate.NONE_CODE.ToString()).ToString();
            SwEditorTracker.TrackEditorEvent(nameof(OnPostUpdate), ESwEditorWisdomLogType.SelfUpdate, ESwEventSeverity.Info, 
                $"Wisdom update completed. Old stage: {oldStageNumber}, Old version: {oldVersionId}, Updated stage: {updatedStageNumber}, Updated version: {updatedVersionId}");
            
            if (!int.TryParse(oldVersionId, out var oldVersion)) return;
            
            const string versionRequiringNewLogin = "7.8.0";
            EditorPrefs.SetBool(SwEditorConstants.SwKeys.MIGRATED_TO_NEW_LOGIN_METHOD, oldVersion < SwUtils.System.ComputeVersionId(versionRequiringNewLogin));
        }
