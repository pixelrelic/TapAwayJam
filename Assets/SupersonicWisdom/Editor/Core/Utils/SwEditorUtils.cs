...
namespace SupersonicWisdomSDK.Editor
{
    public class SwEditorUtils
    {
        ...
        private static void PreOpenSettings()
        {
            if (SwSettings == null)
            {
                CreateSettings();
            ...
            if (!EditorPrefs.GetBool(SwEditorConstants.SwKeys.MIGRATED_TO_NEW_LOGIN_METHOD)) return;
            
            SwEditorAlerts.CustomAlert(SwEditorConstants.UI.TOKEN_MIGRATION_WARNING_MESSAGE, SwEditorConstants.UI.ButtonTitle.CLOSE, 2f);
            EditorPrefs.DeleteKey(SwEditorConstants.SwKeys.MIGRATED_TO_NEW_LOGIN_METHOD);
        }
