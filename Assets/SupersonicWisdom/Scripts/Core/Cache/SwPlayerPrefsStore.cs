    ...
    public class SwPlayerPrefsStore : SwUnmanagedPlayerPrefsStore
    {
        ...
        public override T GetGenericSerializedData<T>(string key, T defaultValue, bool isInternal = true)
        {
            ...
            try
            {
                ...
                {
                    returnValue = JsonConvert.DeserializeObject<T>(dateInString, new JsonSerializerSettings
                    {
            ...
            {
                SwInfra.Logger.LogException(e, EWisdomLogType.Cache, $"{nameof(JsonConvert.DeserializeObject)} failed");
