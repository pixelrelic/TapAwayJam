...
namespace SupersonicWisdomSDK
{
    internal class SwStage10Container : SwCoreContainer
    {
        ...
        public new static ISwContainer GetInstance(Dictionary<string, object> initParamsDictionary)
        {
            ...
            var wisdomNativeAdapter = new SwStage10NativeAdapter(wisdomNativeApi, settingsManager.Settings, userData, testUserState, nativeAdditionalDataAssistant);
            ...
            var dataBridge = new SwStage10DataBridge(userData, wisdomNativeApi, settingsManager.Settings, timerManager);
            ...
            var stageSpecificCustomInitRunnable = new SwAsyncFlow(new[]
            {
                ...
                new SwAsyncFlowStep(appsFlyerAdapter.InitAndStart, 1), // We delay the initialization of AppsFlyer due to a dependency in a remote config value that determines the AF hostname.
            });
            ...
            wisdomNativeAdapter.AddListeners(new List<ISwSessionListener> { sessionListener, timerManager });
            ...
            return new SwStage10Container(initParamsDictionary, mono, filesCacheManager, stageSpecificCustomInitRunnable, settingsManager, readyEventListeners, userStateListeners, configProviders, swAdapters, wisdomNativeAdapter, deepLinkHandler, devTools, userData, tracker, configManager, blockingApiHandler, gameStateSystem, dataBridge, uiToolkitManager, appsFlyerAdapter, facebookAdapter, gameAnalyticsAdapter, timerManager, cvUpdater, revenueCalculator, aliveStatusSender, progressionStatusSender, fpsMeasurementManager, userActiveDay);
        }
